...
export default function GraphClient({ 
  ...
  const processedData = useMemo(() => {
    ...
    const colors = ['#3b82f6', '#14b8a6', '#06b6d4', '#10b981', '#f59e0b', '#ef4444', '#0ea5e9', '#64748b'];
    ...
    return {
      ...
      radarData: departmentData.slice(0, 8)
    };
  }, [initialTickets, slaPolicy, businessHours]);

  const { total, slaBreached, slaPending, totalResolved, slaSuccessRate, slaData, departmentData, typeData, dailyData, staffData, hospitalStats, radarData } = processedData;
    ...
    <div className={`dashboard-container ${theme}`} style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <main className="main-content" style={{ flex: 1, overflowY: 'auto', padding: '2rem 3rem' }}>
        ...
        <div style={{ display: 'grid', gap: '1.5rem', marginBottom: '2rem', width: '100%' }} className="charts-main-grid">
          ...
          {/* Node Load Radar Chart */}
          <div className="chart-block animate-fade-in delay-4" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '20px', padding: '1.5rem' }}>
            <h2 style={{ fontSize: '1.2rem', marginBottom: '1.5rem', fontWeight: 600 }}>🕸️ Node Load Distribution</h2>
            <div style={{ width: '100%', height: '320px' }}>
