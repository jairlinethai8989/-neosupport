...
// --- PDF Dashboard/Analytics Export ---
export const exportAnalyticsPDF = async (elementId: string, title: string, filters?: any) => {
  const element = document.getElementById(elementId);
  if (!element) return;
  ...
  try {
    ...
    // Load Thai Font for metadata text
    await loadThaiFont(pdf);
    try { pdf.setFont('ThaiFont'); } catch(e) {}
    ...
    pdf.setFontSize(18);
    pdf.text("NEO Support Analytics Report", 15, 18);
    
    pdf.setFontSize(10);
    pdf.text(`รายงานสรุปสถิติ: ${title}`, 15, 26);
    pdf.text(`วันที่ออกรายงาน: ${new Date().toLocaleString('th-TH')}`, 15, 32);

    // Filters Summary
    if (filters) {
      pdf.setFontSize(9);
      pdf.text(`หน่วยงาน: ${filters.hospital || 'ทั้งหมด'}  |  ช่วงเวลา: ${filters.period || '30'} วัน  |  มิติ: ${filters.dimension || '-'}`, 15, 38);
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.setFontSize(8);
        pdf.text("Confidential - Generated by NeoMatrix Intelligence Engine", pageWidth / 2, 285, { align: 'center' });
    }
