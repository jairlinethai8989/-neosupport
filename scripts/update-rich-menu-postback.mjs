import fs from 'fs';

// Manual ENV loading for .env.local
const envContent = fs.readFileSync('.env.local', 'utf8');
const TOKEN = envContent.match(/LINE_CHANNEL_ACCESS_TOKEN=(.*)/)?.[1]?.trim();

// 💡 Update this path to your actual image file if needed
const IMAGE_PATH = 'C:/Users/jairlinethai/.gemini/antigravity/brain/c45cd78c-32cb-455a-bb76-0c684a36b9c6/line_rich_menu_neohos_ipd_standard_compact_1773186491561.png';

async function updateRichMenu() {
  if (!TOKEN) {
    console.error("❌ Error: LINE_CHANNEL_ACCESS_TOKEN is missing in .env.local");
    return;
  }

  try {
    console.log("🚀 Starting Professional Rich Menu Update (Postback Flow)...");

    // 1. Define Rich Menu Structure with Postback
    const richMenuData = {
      size: { width: 2500, height: 843 },
      selected: true,
      name: "NEO Support Premium Menu (Postback)",
      chatBarText: "เมนูช่วยเหลือ",
      areas: [
        {
          bounds: { x: 0, y: 0, width: 1250, height: 843 },
          action: { 
            type: "postback", 
            data: "action=knowledge_search",
            displayText: "🔍 ค้นหาวิธีแก้ไข" 
          }
        },
        {
          bounds: { x: 1250, y: 0, width: 1250, height: 843 },
          action: { 
            type: "postback", 
            data: "action=report",
            displayText: "🏥 แจ้งซ่อม" 
          }
        }
      ]
    };

    // 2. Create the new Rich Menu
    const createRes = await fetch('https://api.line.me/v2/bot/richmenu', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${TOKEN}`
      },
      body: JSON.stringify(richMenuData)
    });

    const createData = await createRes.json();
    if (!createData.richMenuId) {
      throw new Error("Failed to create rich menu: " + JSON.stringify(createData));
    }
    
    const richMenuId = createData.richMenuId;
    console.log(`✅ Created Rich Menu: ${richMenuId}`);

    // 3. Upload Image
    if (!fs.existsSync(IMAGE_PATH)) {
        console.warn(`⚠️ Warning: Image not found at ${IMAGE_PATH}. Please upload the image manually to Rich Menu ID: ${richMenuId}`);
    } else {
        const imageBuffer = fs.readFileSync(IMAGE_PATH);
        const uploadRes = await fetch(`https://api-data.line.me/v2/bot/richmenu/${richMenuId}/content`, {
          method: 'POST',
          headers: {
            'Content-Type': 'image/png',
            'Authorization': `Bearer ${TOKEN}`
          },
          body: imageBuffer
        });

        if (!uploadRes.ok) {
          throw new Error("Failed to upload image: " + await uploadRes.text());
        }
        console.log("✅ Uploaded Rich Menu Image");
    }

    // 4. Set as Default for all users
    const defaultRes = await fetch(`https://api.line.me/v2/bot/user/all/richmenu/${richMenuId}`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${TOKEN}` }
    });

    if (!defaultRes.ok) throw new Error("Failed to set default menu: " + await defaultRes.text());
    
    console.log("\n✨ SUCCESS! Your Rich Menu has been upgraded to Postback Mode.");
    console.log(`New Rich Menu ID: ${richMenuId}`);
    console.log("Users will now experience a seamless flow without text messages in chat.");

  } catch (error) {
    console.error("🔴 Setup Failed:", error);
  }
}

updateRichMenu();
